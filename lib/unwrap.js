...
const moduleFunction = function({ nodeRsaKey, secureContainerWorkingDir }) {
	...
	const unwrapFile = ({ inFilePath, outputDirPath, outputName }, callback) => {
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, {unzipOutputTempFilePath, unpackedFileDataList}) => {

				next(err, { ...args, unzipOutputTempFilePath, unpackedFileDataList, outputName });
			};
			...
			decryptData({ nodeRsaKey, fileReadResult,symmetricKeyEncrypted, outputName, secureContainerWorkingDir }, localCallback);
		});
...
console.log(`\n=-=============   UNWRAP  ========================= [unwrap.js.moduleFunction]\n`);

			const details = fileList.filter(fileName=>!fileName.match(/^\./)).map(fileName => {
...
console.log(`resultFilePath=${resultFilePath}`);
				...
				return {
					fileName,
					contents: path.join(resultFilePath, fileName)
				};
			});
...
console.log(`containerSegmentPath=${containerSegmentPath}`);
console.log(`\n=-=============   UNWRAP  ========================= [unwrap.js.moduleFunction]\n`);

		//	fs.cpSync(containerSegmentPath, resultFilePath, { recursive: true });
