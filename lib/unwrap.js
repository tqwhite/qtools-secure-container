...
const moduleFunction = function({ nodeRsaKey }) {
	...
	const unwrapData = ({ nodeRsaKey, fileReadResult, outputName }, callback) => {
		...
		taskList.push((args, next) => {
			...
			const tmp=path.join(dittoOutputFilePath, fileName);
			
console.log(`tmp=${tmp}`);

			return {
				fileName,
				contents: path.join(dittoOutputFilePath, fileName)
			}
			});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			const { unpackedFileDataList, dittoOutputFilePath } = args;
			callback(err, {
	...
	const unwrapFile = ({ inFilePath, outputDirPath, outputName }, callback) => {
		...
		taskList.push((args, next) => {
			const { unpackedFileDataList, outputDirPath, inFilePath, dittoOutputFilePath } = args;
		...
		//		fs.cpSync(fileItem.contents, filePath, {recursive:true});
			...
			const outPath = path.join(outputDirPath, path.basename(dittoOutputFilePath));
			fs.cpSync(dittoOutputFilePath, outPath, {recursive:true});
			fs.rmSync(path.dirname(dittoOutputFilePath), {recursive:true});

			next('', {...args, outputFileList});
		});
		...
		pipeRunner(taskList.getList(), initialData, (err, args) => {
			const { inFilePath, outputDirPath, outputFileList } = args;
