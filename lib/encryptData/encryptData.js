...
const moduleFunction = function({ nodeRsaKey, forceTar }) {
	// ===================================================================================
	//WRAP DATA TO DATA
	...
	const encryptData = (args, callback) => {
		...
		taskList.push((args, next) => {
			...
			const squeezeFlow = require('../squeeze-flow')();
			squeezeFlow.shrink(
				{ secureContainerPath, zipOutputFilePath, forceTar },
				localCallback
			);
		});
		...
		taskList.push((args, next) => {
			const { nodeRsaKey, forceTar, sourceFileInBase64 } = args;
		...
		taskList.push((args, next) => {
			const { zipOutputFilePath, secureContainerPath } = args;
			const localCallback = (err, fileReadResult) => {
				next(err, { ...args, fileReadResult });
			};
			fs.rmSync(secureContainerPath, {recursive:true});
			fs.rm(
		...
		const initialData = { outputName, inFilePath, nodeRsaKey, forceTar, };
