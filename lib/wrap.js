...
const moduleFunction = function({ nodeRsaKey,secureContainerWorkingDir }) {
	...
	const wrapFile = ({ inFilePath, outFilePath }, callback) => {
		console.log(
			`\n=-=============   wrapFile  ========================= [wrap.js.moduleFunction]\n`
		);
...
console.log(`outFilePath=${outFilePath}`);
		...
		taskList.push((args, next) => {
			...
			const localCallback = (err, { encryptedDataInBase64, symmetricKeyEncrypted }) => {
		...
		taskList.push((args, next) => {
			...
			const tmp={
				...
				fileList:[
					{[fileName]:encryptedDataInBase64}
				]
			};
		...
		taskList.push((args, next) => {
			const { encryptedDataInBase64, inFilePath, outFilePath, container } = args;
...
console.log(`WRAP outFilePath=${outFilePath}`);
		...
		const initialData = { inFilePath, outFilePath, nodeRsaKey, secureContainerWorkingDir };
