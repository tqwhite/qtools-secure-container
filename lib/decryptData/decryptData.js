...
const symmetricKey = 'Pp67jE9thlEkC9FxEIdw173PkSs8D7BbWxnKLCBkirk=';
...
const moduleFunction = function({ nodeRsaKey }) {
	...
	const decryptData = (
		{ nodeRsaKey, fileReadResult, symmetricKeyEncrypted, outputName, secureContainerWorkingDir },
		callback
	) => {
		console.log(
			`\n=-=============   decryptData  ========================= [decryptData.js.moduleFunction]\n`
		);
		...
		taskList.push((args, next) => {
			...
			const tmp = Buffer.from(
				symmetricKeyEncrypted.toString('utf8'),
				'base64'
			);
			
			const symmetricKeyDecrypted = nodeRsaKey
				.decrypt(tmp)
		...
		taskList.push((args, next) => {
			...
			const zipFilePath = path.join(secureContainerWorkingDir, path.basename(outputName) + '.zip');
...
console.log(`unzipRawOutputDirPath=${unzipRawOutputDirPath}`);
		...
		taskList.push((args, next) => {
			...
			const payloadDirPath = path.join(unzipRawOutputDirPath,
			'CONTAINER');
...
console.log(`\npayloadDirPath=${payloadDirPath}`);
console.log(`unzipOutputTempFilePath=${unzipOutputTempFilePath}`);


			fs.cp(payloadDirPath, unzipOutputTempFilePath, {recursive:true}, localCallback);
		});

		//-----------------------------------------------------------------------------------
		// EXECUTE AND EXIT

		const initialData = { nodeRsaKey, fileReadResult, symmetricKeyEncrypted, outputName };
